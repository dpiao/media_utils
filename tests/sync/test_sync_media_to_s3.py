"""
Tests for sync.app.py

Run with:  pytest test_sync.app.py -v
"""
...
def test_s3_key_simple() -> None:
    root = Path("/Movies")
    file = Path("/Movies/Action/foo.mkv")
...
def test_s3_key_nested() -> None:
    root = Path("/Movies")
    file = Path("/Movies/Drama/2024/bar.mkv")
    key = sut.s3_key(root, file, "s3://park.movies.archive/")
...
def test_upload_file_builds_correct_command(tmp_path) -> None:
    ...
    with patch("sync.app.run_aws") as mock_run:
        sut.upload_file(tmp_path, f, "s3://park.movies.archive/", dry_run=False, ignore=IgnoreRules.empty())
...
def test_upload_file_skips_ignored(tmp_path) -> None:
    ...
    f.touch()
    ignore = IgnoreRules(["V Vam My Render/202?????-??????*"], [])
...
def _make_handler(tmp_path: Path, ignore: IgnoreRules | None = None) -> tuple[sut.MediaHandler, MagicMock]:
    checker = MagicMock()
    handler = sut.MediaHandler(
        tmp_path, "s3://park.movies.archive/", checker, ignore or IgnoreRules.empty()
    )
...
def test_stability_checker_uploads_after_stable(tmp_path) -> None:
    ...
    with patch("sync.app.upload_file", side_effect=lambda root, p, *a, **k: uploaded.append(str(p))):
        checker = sut.StabilityChecker(stable_secs=1, dry_run=False, ignore=IgnoreRules.empty())
...
def test_stability_checker_duplicate_enqueue_logs_once(capsys, tmp_path) -> None:
    f = tmp_path / "movie.mkv"
    f.write_bytes(b"x" * 100)
    checker = sut.StabilityChecker(stable_secs=60, dry_run=False, ignore=IgnoreRules.empty())
    checker.enqueue(f, tmp_path, "s3://park.movies.archive/")
    checker.enqueue(f, tmp_path, "s3://park.movies.archive/")
    out = capsys.readouterr().out
...
def test_stability_checker_does_not_upload_while_growing(tmp_path) -> None:
    ...
    with patch("sync.app.upload_file", side_effect=lambda p, *a, **k: uploaded.append(str(p))):
        checker = sut.StabilityChecker(stable_secs=2, dry_run=False, ignore=IgnoreRules.empty())
        checker.enqueue(f, tmp_path, "s3://park.movies.archive/")
...
def test_stability_checker_dry_run_passes_flag(tmp_path) -> None:
    ...
    with patch("sync.app.upload_file", side_effect=capture):
        checker = sut.StabilityChecker(stable_secs=1, dry_run=True, ignore=IgnoreRules.empty())
        checker.enqueue(f, tmp_path, "s3://park.movies.archive/")
        time.sleep(3)
...
def test_initial_sync_excludes_temp_extensions(tmp_path) -> None:
    (tmp_path / "Keep").mkdir()
    with patch("sync.app.run_aws") as mock_run:
        sut.initial_sync(tmp_path, "s3://park.movies.archive/", dry_run=True, ignore=IgnoreRules.empty())
...
def test_ignore_regex() -> None:
    ignore = IgnoreRules([], [__import__("re").compile(r"V Vam My Render/\d{8}-\d{6}")])
...
def test_ignore_glob_anywhere() -> None:
    ignore = IgnoreRules(["**/Thumbs.db"], [])
    assert ignore.is_ignored("some/nested/Thumbs.db")


def test_ignore_load_from_file(tmp_path) -> None:
    ignore_file = tmp_path / "sync.app.ignore"
    ignore_file.write_text("# comment\nfoo/*.part\nre: bar/\\d+\n", encoding="utf-8")
    ignore = IgnoreRules.load(ignore_file)
...
def test_initial_sync_skips_excluded_top_level_dirs(tmp_path, capsys) -> None:
    (tmp_path / "TV").mkdir()
    (tmp_path / "Keep").mkdir()
    ...
    with patch("sync.app.run_aws") as mock_run:
        sut.initial_sync(tmp_path, "s3://park.movies.archive/", dry_run=True, ignore=ignore)
    # Only Keep is synced
    synced_paths = [c.args[0][3] for c in mock_run.call_args_list]
    assert any(str(tmp_path / "Keep") == p for p in synced_paths)
    assert not any(str(tmp_path / "TV") == p for p in synced_paths)
